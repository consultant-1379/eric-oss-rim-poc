...
ARG USER_NAME="eric-oss-rim-poc"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG APP_MODULE=eric-oss-rim-poc-app
ARG APP_VERSION
ADD ${APP_MODULE}/target/${APP_MODULE}-${APP_VERSION}.jar eric-oss-rim-poc.jar
...
LABEL \
    org.opencontainers.image.title=eric-oss-rim-poc-jsb \
