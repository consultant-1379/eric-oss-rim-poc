...
checkServiceYAML(){
    SERVICE_NAME=$1
    if grep -q "include \"$SERVICE_NAME.prometheus\" ." ./charts/$SERVICE_NAME/templates/service.yaml; then
        ...
        echo -e "What is needed:"
        echo -e "include \"$SERVICE_NAME.prometheus\" ."
        echo "false" >> .bob/var.metrics-exposed;
    fi
}
...
checkDeploymentYAML(){
    SERVICE_NAME=$1
    if grep -q "include \"$SERVICE_NAME.prometheus\" ." ./charts/$SERVICE_NAME/templates/deployment.yaml; then
...
checkPomXML(){
    ...
    grep -B2 -A7 "<artifactId>micrometer-core</artifactId>" ./eric-oss-rim-poc-app/pom.xml | sed 's/^[ \t]*//;s/[ \t]*$//' > .bob/var.scrapedLine || true;
...
checkCoreApplicationJAVA(){
    ...
    echo -e "public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder, final OkHttpClient okHttpClient)" > .bob/var.compareToLine;
    echo -e "return restTemplateBuilder.requestFactory(() -> new OkHttp3ClientHttpRequestFactory(okHttpClient)).build()" >> .bob/var.compareToLine;

    find ./eric-oss-rim-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder, final OkHttpClient okHttpClient)' {} \; > .bob/var.scrapedLine || true;
    find ./eric-oss-rim-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'return restTemplateBuilder.requestFactory(() -> new OkHttp3ClientHttpRequestFactory(okHttpClient)).build()' {} \; >> .bob/var.scrapedLine || true;
